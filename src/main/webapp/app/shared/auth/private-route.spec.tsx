...
describe('hasAnyAuthority', () => {
  ...
  it('Should return false when authorities is invalid', () => {
    ...
    expect(hasAnyAuthority([], [AUTHORITIES.EMPLOYEE])).toEqual(false);
  });

  it('Should return true when authorities is valid and hasAnyAuthorities is empty', () => {
    expect(hasAnyAuthority([AUTHORITIES.EMPLOYEE], [])).toEqual(true);
  });

  it('Should return true when authorities is valid and hasAnyAuthorities contains an authority', () => {
    expect(hasAnyAuthority([AUTHORITIES.EMPLOYEE], [AUTHORITIES.EMPLOYEE])).toEqual(true);
    expect(hasAnyAuthority([AUTHORITIES.EMPLOYEE, AUTHORITIES.ADMIN], [AUTHORITIES.EMPLOYEE])).toEqual(true);
    expect(hasAnyAuthority([AUTHORITIES.EMPLOYEE, AUTHORITIES.ADMIN], [AUTHORITIES.EMPLOYEE, AUTHORITIES.ADMIN])).toEqual(true);
    expect(hasAnyAuthority([AUTHORITIES.EMPLOYEE, AUTHORITIES.ADMIN], [AUTHORITIES.EMPLOYEE, 'ROLEADMIN'])).toEqual(true);
    expect(hasAnyAuthority([AUTHORITIES.EMPLOYEE, AUTHORITIES.ADMIN], [AUTHORITIES.ADMIN])).toEqual(true);
  });

  it('Should return false when authorities is valid and hasAnyAuthorities does not contain an authority', () => {
    expect(hasAnyAuthority([AUTHORITIES.EMPLOYEE], [AUTHORITIES.ADMIN])).toEqual(false);
    expect(hasAnyAuthority([AUTHORITIES.EMPLOYEE, AUTHORITIES.ADMIN], ['ROLE_USERSS'])).toEqual(false);
    expect(hasAnyAuthority([AUTHORITIES.EMPLOYEE, AUTHORITIES.ADMIN], ['ROLEUSER', 'ROLEADMIN'])).toEqual(false);
  });
});
